...
def genData(n):
    ...
    dataDual = np.genfromtxt('dual.csv', delimiter=',', names=['size', 'comps', 'swaps', 'time'])
    ...
    for i in range(1, 100):
        ...
        sizeDual.append(dataDual['size'][i*n-1])
        timeDual.append(np.mean(dataDual['time'][(n-1)*i:n*i])/1000000)
        compsDual.append(np.mean(dataDual['comps'][n*(i-1):n*i]))
        swapsDual.append(np.mean(dataDual['swaps'][(n-1)*i:n*i]))
    ...
    for i in range(0, 99):
        ...
        cnDual.append(compsDual[i] / sizeDual[i])
        snDual.append(swapsDual[i] / sizeDual[i])
    ...
    plt.plot(sizeDual, timeDual, label='Dual')
    plt.xlabel('size')
    plt.ylabel('time(ms)')
    ...
    plt.plot(sizeDual, compsDual, label='Dual')
    plt.xlabel('size')
    plt.ylabel('comps')
    ...
    plt.plot(sizeDual, swapsDual, label='Dual')
    plt.xlabel('size')
    plt.ylabel('swaps')
    ...
    plt.plot(sizeDual, cnDual, label='Dual')
    plt.xlabel('size')
    ...
    plt.plot(sizeDual, snDual, label='Dual')
    plt.xlabel('size')
