...
public class Zad3 {
    private static final Set<String> types = new HashSet<String>(Arrays.asList(new String[] {"insert", "merge", "quick", "dual"}));
                    ...
                    for(int n = 1; n <= 100; n++) {
                        for(int i = 0; i < k; i++) {
                            ...
                            if(!Sort.isSorted(tab, comp)) {
                                System.out.println("ERROR array in not sorted!");
                                return;
                            }
                            stats.setTime(stop - start);
                            // System.out.println(time); //nanosec time
                            out.println(n*100 + "," + stats.getComps() + "," + stats.getSwaps() + "," + stats.getTime() + " ns");
                            stats.clearStats();
                        }
                    }
                    out.close();
